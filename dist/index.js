...
var StateBackend = class {
	...
	*/
	write(filePath, content) {
		const files = this.getFiles();
		if (filePath in files) return { error: `Cannot write to ${filePath} because it already exists. Read and then make an edit, or write to a new path.` };
		const newFileData = createFileData(content);
		return {
			path: filePath,
			filesUpdate: {
				...files,
				[filePath]: newFileData
			}
		};
	}
	/**
